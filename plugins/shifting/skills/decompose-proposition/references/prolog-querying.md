...
Patterns and modules for interrogating `existing-world.pl` (or any Prolog facts file) during the evidence-gathering phase. The `decompose-proposition` skill delegates these queries to the `shifting:agent-of-questions` sub-agent — this reference exists for spot-checks and for the agent's own use.
