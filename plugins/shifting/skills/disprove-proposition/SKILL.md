...
- **English proposition** → delegate the sharpening pass to `shifting:proposition-sharpener` *before* adversary routing. Brief it with `raw_proposition_text` (the user's input verbatim), `existing_world_path` (the supplied `existing-world.pl` or whatever KB scopes the claim), and `prolog_introspect_path = ${CLAUDE_SKILL_DIR}/../../prolog/introspect`. Two return shapes: `outcome: "sharpened"` produces a one-sentence falsifiable form the orchestrator now treats as the pinned `target_text`, then routes to whichever adversary the formalism maps onto; `outcome: "abstained"` returns the skill-level verdict `abstained` immediately, with the sharpener's `reason` and `what_user_should_clarify` surfaced in `disprove_obstruction/2`. Do not paper over an abstention by guessing a sharpening inline — the agent already considered and rejected every reading that exceeded evidence support.
